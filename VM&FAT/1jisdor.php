...
$pageTitle = "Jisdor - MLP Logistic";

/**
 * BI webservice (wskursbi.asmx), HTTP GET form
 * Method: getSubKursJisdor3(mts, startdate, enddate)
 * Date format: yyyy-mm-dd
 */
function fetchJisdor($mts, $startDate, $endDate)
{
  $url = 'https://www.bi.go.id/biwebservice/wskursbi.asmx/getSubKursJisdor3?' . http_build_query([
    'mts'       => $mts,
    'startdate' => $startDate,
    'enddate'   => $endDate,
  ]);
  ...
  $rows = [];
  foreach ($tables as $t) {
    $tgl  = $xp->query('.//*[local-name()="tgl_subkursasing"]', $t)->item(0);
    $kurs = $xp->query('.//*[local-name()="beli_subkursasing"]', $t)->item(0);
    if (!$tgl || !$kurs) continue;
    $rows[] = [
      'tanggal' => $tgl->textContent,
      'kurs'    => (float)$kurs->textContent,
    ];
  }
  return $rows;
}

function fetchJisdorCached($mts, $startDate, $endDate, $ttlSeconds = 900)
{
  $cacheDir = __DIR__ . '/cache';
  if (!is_dir($cacheDir)) @mkdir($cacheDir, 0775, true);

  $cacheFile = $cacheDir . '/jisdor_' . md5($mts . '|' . $startDate . '|' . $endDate) . '.json';
  ...
  $data = fetchJisdor($mts, $startDate, $endDate);
  if ($data !== null) {
...
$fetch  = fetchJisdorCached($mts, $startDate->format('Y-m-d'), $endDate->format('Y-m-d'));
      ...
      <table class="table mb-0 align-middle">
        <thead>
          <tr>
            ...
            <th class="py-3 px-4 text-white" style="background:#0d4d94;">Kurs</th>
          </tr>
        </thead>
        <tbody>
          <?php if ($shouldAutoReload): ?>
            <tr><td colspan="2" class="text-center text-muted py-4">Loading&hellip;</td></tr>
          <?php elseif ($fetchFailed): ?>
            <tr><td colspan="2" class="text-center text-muted py-4">Gagal memuat data. <a href="?<?= htmlspecialchars(http_build_query(array_merge($_GET, ['retry' => 0]))) ?>">Coba lagi</a>.</td></tr>
          ...
          <?php else: foreach ($pageData as $row): ?>
            <tr>
              ...
              <td class="px-4"><?= htmlspecialchars(formatRupiah($row['kurs'])) ?></td>
            </tr>
