...
// VM&FAT/2kurstengah.php only checks that a session exists — same
// intentional server-side tightening as jetty/jisdor (see
// jetty-gate.node-only.spec.ts), applied here to kurs-tengah.
describe("kurs-tengah access gate — divisi restriction (Node app only)", () => {
  ...
  it("blocks a non-IT, non-VM&FAT divisi with 403 on the page route", async () => {
    ...
    const res = await client.get(node.paths.kursTengah);
