...
// logged-in user of any divisi can hit 6flf.php directly; the IT-or-
...
// applied here to flf (issue #8), so this assertion only holds for the
// Node target.
describe("flf access gate — divisi restriction (Node app only)", () => {
  ...
  it("blocks a non-IT, non-Operation divisi with 403 on the page route", async () => {
    ...
    const res = await client.get(node.paths.flf);
  ...
  it("blocks a non-IT, non-Operation divisi with 403 on the AJAX endpoint", async () => {
    ...
    const res = await client.postForm(`${node.paths.flf}?ajax=1`, {
      action: "list",
    });
    expect(res.status).toBe(403);
  });
