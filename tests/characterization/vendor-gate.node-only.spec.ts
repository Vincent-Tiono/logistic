...
// Same intentional Node-side tightening as vessel (#3) and shipper (#4):
// PHP's operation_bootstrap.php only checks that a session exists, so any
// logged-in user of any divisi can hit 3vendor.php directly. The IT-or-
// Operation rule is purely a sidebar-link-visibility gate in PHP, not
// enforced server-side, so this assertion only holds for the Node target.
describe("vendor access gate — divisi restriction (Node app only)", () => {
  ...
  it("blocks a non-IT, non-Operation divisi with 403 on the page route", async () => {
    ...
    const res = await client.get(node.paths.vendor);
  ...
  it("blocks a non-IT, non-Operation divisi with 403 on the AJAX endpoint", async () => {
    ...
    const res = await client.postForm(`${node.paths.vendor}?ajax=1`, {
      action: "list",
    });
    expect(res.status).toBe(403);
  });
