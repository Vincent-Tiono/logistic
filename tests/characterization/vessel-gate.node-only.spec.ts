...
// logged-in user of any divisi can hit 1vessel.php directly; the IT-or-
// Operation rule is purely a sidebar-link-visibility gate in PHP, not
// enforced server-side. Issue #3 asks the Node port to enforce that gate
// server-side, an intentional tightening beyond raw PHP behavior, so this
// assertion only holds for the Node target.
describe("vessel access gate — divisi restriction (Node app only)", () => {
  ...
  it("blocks a non-IT, non-Operation divisi with 403 on the page route", async () => {
    ...
    const res = await client.get(node.paths.vessel);
  ...
  it("blocks a non-IT, non-Operation divisi with 403 on the AJAX endpoint", async () => {
    ...
    const res = await client.postForm(`${node.paths.vessel}?ajax=1`, {
      action: "list",
    });
    expect(res.status).toBe(403);
  });
