...
// logged-in user of any divisi can hit 5jetty.php directly; the IT-or-
// Operation rule is purely a sidebar-link-visibility gate in PHP, not
// enforced server-side. Same intentional tightening as shipper (issue #4),
// applied here to jetty (issue #7), so this assertion only holds for the
// Node target.
describe("jetty access gate — divisi restriction (Node app only)", () => {
  ...
  it("blocks a non-IT, non-Operation divisi with 403 on the page route", async () => {
    ...
    const res = await client.get(node.paths.jetty);
  ...
  it("blocks a non-IT, non-Operation divisi with 403 on the AJAX endpoint", async () => {
    ...
    const res = await client.postForm(`${node.paths.jetty}?ajax=1`, {
      action: "list",
    });
    expect(res.status).toBe(403);
  });
