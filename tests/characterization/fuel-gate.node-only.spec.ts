...
// VM&FAT/3fuel.php only checks that a session exists — same intentional
// server-side tightening as jetty/jisdor/kurs-tengah (see
// jetty-gate.node-only.spec.ts), applied here to fuel.
describe("fuel access gate — divisi restriction (Node app only)", () => {
  ...
  it("blocks a non-IT, non-VM&FAT divisi with 403 on the page route", async () => {
    ...
    const res = await client.get(node.paths.fuel);
    expect(res.status).toBe(403);
  });

  it("blocks a non-IT, non-VM&FAT divisi with 403 on the AJAX save endpoint", async () => {
    const client = new HttpClient(node.baseUrl);
    await client.postForm(node.paths.login, {
      username: otherUser,
      password: otherPassword,
    ...
    const res = await client.postForm(node.paths.fuel, {
      action: "save_fuel",
      bulan_tahun: "Jan-99",
      periode: "1",
      field: "pertamina",
