...
// logged-in user of any divisi can hit 9coalbarging.php directly; the IT-or-
// Operation rule is purely a sidebar-link-visibility gate in PHP, not
// enforced server-side. Same intentional tightening as TLU Operation (issue
// #10), applied here to Coal Barging (issue #13), so this assertion only
// holds for the Node target.
describe("coal-barging access gate — divisi restriction (Node app only)", () => {
  ...
  it("blocks a non-IT, non-Operation divisi with 403 on the page route", async () => {
    ...
    const res = await client.get(node.paths.coalBarging);
  ...
  it("blocks a non-IT, non-Operation divisi with 403 on the AJAX endpoint", async () => {
    ...
    const res = await client.get(
      `${node.paths.coalBarging}?ajax=1&action=si_barges_by_vessel&no_pk=X`
    );
    expect(res.status).toBe(403);
  });
});
