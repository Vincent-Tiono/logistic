...
import { VESSEL_TEMPLATE_COLUMNS } from "../../src/services/vessel.service.js";
...
describe.each(targets)("vessel — $name", (target) => {
  ...
  describe("download=vessel_template", () => {
    it("returns the template CSV with the expected header and filename", async () => {
      const client = await loginAs(opUser, opPassword);
      const res = await client.get(`${target.paths.vessel}?download=vessel_template`);
      expect(res.status).toBe(200);
      expect(res.contentType).toContain("text/csv");
      expect(res.contentDisposition).toContain("vessel_template.csv");
      ...
      expect(res.body).toContain(VESSEL_TEMPLATE_COLUMNS.join(","));
    });
  });
});
