...
// logged-in user of any divisi can hit 7sibarges.php directly; the IT-or-
// Operation rule is purely a sidebar-link-visibility gate in PHP, not
// enforced server-side. Same intentional tightening as jetty (issue #7),
// applied here to sibarges (issue #9), so this assertion only holds for the
// Node target.
describe("sibarges access gate — divisi restriction (Node app only)", () => {
  ...
  it("blocks a non-IT, non-Operation divisi with 403 on the page route", async () => {
    ...
    const res = await client.get(node.paths.sibarges);
  ...
  it("blocks a non-IT, non-Operation divisi with 403 on the AJAX endpoint", async () => {
    ...
    const res = await client.postForm(`${node.paths.sibarges}?ajax=1`, {
      action: "list",
    });
    expect(res.status).toBe(403);
  });
